...
import { getSelectedSiteId, getSelectedSiteName, getAdminIdForApi, notifySiteChange } from "../../core/utils/apiHelpers";

interface Site {
  id: string | number; // UUIDs are strings, but can be number for backward compatibility
  site_name: string;
  address: string;
  city: string;
  state: string;
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Only fetch sites if user is admin
    if (role === "admin") {
  ...
  const fetchSites = async () => {
    try {
      ...
      if (response.data.status === 200 && response.data.data) {
        ...
        console.log("Sites loaded:", response.data.data.length);
        
        const savedSiteId = getSelectedSiteId();
        
        if (savedSiteId) {
          // Verify saved site still exists (compare as strings - UUIDs are strings)
          const siteExists = response.data.data.some(s => String(s.id) === String(savedSiteId));
          if (siteExists) {
            // Update selected site name if it exists
            const site = response.data.data.find(s => String(s.id) === String(savedSiteId));
            if (site) {
            ...
            handleSiteChange(firstSite.id, true);
          }
        } else if (response.data.data.length > 0) {
        ...
        console.warn("No sites found or invalid response");
        setSites([]);
      }
    } catch (error: any) {
  ...
  return (
    <>
      ...
      <div className="page-wrapper">
        {/* Site Selection Bar - Top of Dashboard */}
        {userRole === "admin" && (
          <div className="bg-primary text-white p-2" style={{ position: "sticky", top: 0, zIndex: 1000 }}>
            <div className="container-fluid">
              <div className="d-flex justify-content-between align-items-center">
                <div className="d-flex align-items-center gap-3">
                  <i className="feather-map-pin fs-5"></i>
                  <span className="fw-semibold">Select Site:</span>
                  {loadingSites ? (
                    ...
                    >
                      {sites.map((site) => (
                        <option key={site.id} value={site.id} style={{ backgroundColor: "#007bff", color: "#fff" }}>
                          {site.site_name}
                        </option>
                      ))}
                    </select>
                ...
                <div className="modal-body">
                  ...
                  <div style={{ maxHeight: "400px", overflowY: "auto" }}>
                    {loadingSites ? (
                      ...
                      <div className="text-center p-4">
                        <i className="feather-alert-circle" style={{ fontSize: "48px", color: "#ccc" }}></i>
                        <p className="mt-2 text-muted">No sites available</p>
                      </div>
                    ) : (
                          ...
                          .map((site) => (
                            ...
                            >
                              <input
                                ...
                                name="siteSelection"
                                checked={selectedSiteId !== null && String(selectedSiteId) === String(site.id)}
                                onChange={() => {
                                  // Automatically select site when clicked (no need to click "Set" button)
