...
const ContactModal: React.FC<ContactModalProps> = ({
  ...
  const handleExtractFromImage = async () => {
    ...
    try {
      ...
      let url = role === "user" && userId
        ? `http://127.0.0.1:8000/api/contact/contact-extract-by-user/${siteId}/${userId}/`
        : `http://127.0.0.1:8000/api/contact/contact-extract/${siteId}/`;
    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        ...
        if (key === "business_card_image") {
          ...
          // Always preserve source_type, especially when editing
          if (editingContact && editingContact.source_type) {
            // Preserve original source_type when editing
            formDataToSend.append(key, editingContact.source_type);
          } else if (value) {
      ...
      if (editingContact) {
        ...
        // Organization role: admin_id should be passed as query param
        const role = sessionStorage.getItem("role");
        let url = `http://127.0.0.1:8000/api/contact/contact-detail-update-delete/${siteId}/${updateUserId}/${editingContact.id}/`;
        if (role === "organization" && adminId) {
          url += `?admin_id=${adminId}`;
        }
        ...
        // Organization role: admin_id should be passed as query param
        let url = role === "user" && userId
          ? `http://127.0.0.1:8000/api/contact/contact-list-create-by-user/${siteId}/${userId}/`
          : `http://127.0.0.1:8000/api/contact/contact-list-create/${siteId}/`;
