...
const OrganizationDashboard = () => {
  ...
  useEffect(() => {
    if (selectedAdminForDashboard) {
      const adminUserId = selectedAdminForDashboard.user?.id || selectedAdminForDashboard.user_id;
      if (adminUserId) {
        fetchSitesForAdmin(adminUserId);
        // Load saved site
        const savedSiteId = sessionStorage.getItem("selected_site_id");
        if (savedSiteId) {
          setSelectedSiteId(parseInt(savedSiteId));
          const savedSiteName = sessionStorage.getItem("selected_site_name");
          if (savedSiteName) {
  ...
  const fetchSitesForAdmin = async (adminUserId: string) => {
    try {
      ...
      const response = await axios.get<{
        ...
        message: string;
        data: any[];
      }>(`${BACKEND_PATH}admin/sites/?admin_id=${adminUserId}`, getAuthHeaders());
      ...
      if (response.data.status === 200 && response.data.data) {
        ...
        if (savedSiteId) {
          const siteExists = response.data.data.some((s: any) => s.id === parseInt(savedSiteId));
          if (siteExists) {
            const site = response.data.data.find((s: any) => s.id === parseInt(savedSiteId));
            if (site) {
              setSelectedSiteId(site.id);
              setSelectedSiteName(site.site_name);
            }
          } else if (response.data.data.length > 0) {
            // Site doesn't exist, select first one
            handleSiteChange(response.data.data[0].id, response.data.data[0].site_name);
          }
        } else if (response.data.data.length > 0) {
          // No site selected, select first one as default
          handleSiteChange(response.data.data[0].id, response.data.data[0].site_name);
        }
      } else {
        setSites([]);
  ...
  const handleSiteChange = (siteId: number, siteName: string) => {
    setSelectedSiteId(siteId);
    setSelectedSiteName(siteName);
    // Use notifySiteChange to update sessionStorage and dispatch event
    notifySiteChange(siteId.toString(), siteName);
    toast.success(`Site selected: ${siteName}`);
  };

  const handleAddAdmin = async () => {
    // Validate form
    if (!addAdminForm.admin_name || !addAdminForm.email || !addAdminForm.username || !addAdminForm.password || !addAdminForm.phone_number || !addAdminForm.state || !addAdminForm.city) {
    ...
    try {
      ...
      const response = await axios.post(
        `${BACKEND_PATH}register/admin`,
        {
          ...
          state: addAdminForm.state,
          city: addAdminForm.city,
          user: {
      ...
      if (error.response?.data) {
        ...
        if (errorData.errors) {
          ...
          // Handle other top-level errors (like phone_number, admin_name, etc.)
  ...
  const handleToggleAdminStatus = async (admin: Admin) => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}admin/`;
      if (role === "organization" && admin.id) {
        url += `?admin_id=${admin.id}`;
      }
                  ...
                  <div className="row">
                    ...
                    <div className="col-md-6 mb-3">
                      ...
                      <select
                        ...
                        value={addAdminForm.state}
                        onChange={(e) =>
                          setAddAdminForm({ ...addAdminForm, state: e.target.value })
                        }
                        required
                      >
                    ...
                    <div className="col-md-6 mb-3">
                      ...
                      <input
                        ...
                        value={addAdminForm.city}
                        onChange={(e) =>
                          setAddAdminForm({ ...addAdminForm, city: e.target.value })
                        }
                        required
                      />
                    </div>
