...
const ApplyLeaveModal: React.FC<ApplyLeaveModalProps> = ({ employee, onLeaveApplied }) => {
  ...
  useEffect(() => {
    if (fromDate && toDate) {
      ...
      if (diffDays === 1 && leaveDayType !== "full_day") {
        setTotalDays(0.5);
      } else {
  ...
  const fetchLeaveTypes = async () => {
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}/api/leave-types/${site_id}/`;
  ...
  const fetchLeaveBalances = async () => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}/api/leave-balances/${site_id}/${employeeUserId}/?year=${currentYear}`;
      if (role === "organization" && adminId) {
        url += `&admin_id=${adminId}`;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedLeaveType || !fromDate || !toDate || !leaveDayType || !reason) {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}/api/leave-applications/${site_id}/${employeeUserId}/`;
      if (role === "organization" && adminId) {
        url += `?admin_id=${adminId}`;
      }
            ...
            <div className="modal-body">
              ...
              <div className="mb-3">
                ...
                <select
                  className="form-select"
                  value={leaveDayType}
                  onChange={(e) => setLeaveDayType(e.target.value as "full_day" | "first_half" | "second_half")}
                  required
                ...
                {fromDate && toDate && fromDate !== toDate && (
                  <small className="text-muted d-block mt-1">
                    Note: Half day options are only applicable for single day leaves
                  </small>
                )}
              </div>
