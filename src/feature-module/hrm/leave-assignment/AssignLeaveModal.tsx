...
const AssignLeaveModal: React.FC<AssignLeaveModalProps> = ({
  ...
  const fetchExistingBalances = async () => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `http://127.0.0.1:8000/api/leave-balances/${site_id}/${employeeUserId}/?year=${year}`;
      if (role === "organization" && admin_id) {
        url += `&admin_id=${admin_id}`;
      }
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      // URL should only have site_id and user_id (employee_id), not admin_id
      const response = await axios.post(
        `http://127.0.0.1:8000/api/assign-leaves/${site_id}/${employeeUserId}/`,
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleEditSave = async (leaveTypeId: number) => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `http://127.0.0.1:8000/api/leave-balances/${site_id}/${employeeUserId}/${existingBalance.id}/`;
      if (role === "organization" && admin_id) {
        url += `?admin_id=${admin_id}`;
      }

      const response = await axios.put(url, { assigned: newValue }, {
        headers: { Authorization: `Bearer ${token}` },
      });
