...
const AssignShiftModal: React.FC<AssignShiftModalProps> = ({
  ...
  const fetchShifts = async () => {
    try {
      ...
      let url = `http://127.0.0.1:8000/api/service-shifts/${site_id}/`;
  ...
  const fetchAssignedShifts = async () => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `http://127.0.0.1:8000/api/assign-shifts/${site_id}/${employeeUserId}/`;
      if (role === "organization" && admin_id) {
        url += `?admin_id=${admin_id}`;
      }
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      const response = await axios.post(url, { shift_ids: selectedShifts }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUnassignSingle = async (shiftId: number, shiftName: string) => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let deleteUrl = `http://127.0.0.1:8000/api/assign-shifts/${site_id}/${employeeUserId}/${shiftId}/`;
      if (role === "organization" && admin_id) {
