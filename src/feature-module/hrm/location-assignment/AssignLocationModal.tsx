...
const AssignLocationModal: React.FC<AssignLocationModalProps> = ({
  ...
  const fetchAssignedLocations = async () => {
    ...
    try {
      ...
      // Organization role: admin_id should be passed as query param
      const role = sessionStorage.getItem("role");
      let url = `http://127.0.0.1:8000/api/assign-locations/${site_id}/${employeeUserId}/`;
      if (role === "organization" && admin_id) {
        url += `?admin_id=${admin_id}`;
      }
  ...
  const handleUnassignSingle = async (locationId: number, locationName: string) => {
    ...
    try {
      ...
      // Organization role: admin_id should be passed as query param
      const role = sessionStorage.getItem("role");
      let deleteUrl = `http://127.0.0.1:8000/api/assign-locations/${site_id}/${employeeUserId}/${locationId}/`;
      if (role === "organization" && admin_id) {
