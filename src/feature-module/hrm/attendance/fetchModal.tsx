...
import { img_path, BACKEND_PATH } from '../../../environment';
...
const AttendanceAdmin = () => {
  ...
  const fetchAttendanceData = useCallback(async () => {
    ...
    try {
      ...
      // Get site_id from sessionStorage (required)
      const site_id = sessionStorage.getItem("selected_site_id");
      
      if (!site_id) {
        const role = sessionStorage.getItem("role");
        console.error("Site ID not found in session storage.");
        if (role === "organization") {
          toast.error("Please select a site first from the dashboard.");
        } else {
      ...
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}employee-attendance/${site_id}/?date=${dateParam}&page=${currentPage}&page_size=${pageSize}`;
      ...
      if (jsonData && jsonData.data && jsonData.data.length > 0) {
        ...
        const mappedData = jsonData.data.map((item: any) => ({
          ...
          CustomEmployeeId: item.custom_employee_id || item.employee_id,
          Role: 'Employee',
      ...
      if (jsonData && jsonData.pagination) {
  ...
  }, [selectedDate, statusFilter, searchQuery, currentPage, pageSize]);
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a: any, b: any) => (a.CustomEmployeeId || '').localeCompare(b.CustomEmployeeId || ''),
    },
    {
      title: "Status",
      dataIndex: "Status",
  ...
  const handleExportExcel = async () => {
    try {
      ...
      let url = `${BACKEND_PATH}employee-attendance/${site_id}/?date=${dateParam}&export=true`;
      if (role === "organization" && admin_id) {
        url += `&admin_id=${admin_id}`;
      }
        ...
        <div className="content">
          ...
          <div className="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
            <div className="my-auto mb-2">
              <h2 className="mb-1">Attendance Employee</h2>
            </div>
                  ...
                  {pagination.total_items > 0 && (
                    <div className="d-flex align-items-center justify-content-between p-3 border-top">
                      <div className="d-flex align-items-center">
                        <span className="me-3">Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, pagination.total_items)} of {pagination.total_items} entries</span>
                        <select 
