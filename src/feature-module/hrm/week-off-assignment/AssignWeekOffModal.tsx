...
const AssignWeekOffModal: React.FC<AssignWeekOffModalProps> = ({
  ...
  const fetchAssignedWeekOffs = async () => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `http://127.0.0.1:8000/api/assign-week-offs/${site_id}/${employeeUserId}/`;
      if (role === "organization" && admin_id) {
        url += `?admin_id=${admin_id}`;
      }
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      const response = await axios.post(url, { week_off_ids: selectedWeekOffs }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUnassignSingle = async (weekOffId: number, weekOffName: string) => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let deleteUrl = `http://127.0.0.1:8000/api/assign-week-offs/${site_id}/${employeeUserId}/${weekOffId}/`;
      if (role === "organization" && admin_id) {
