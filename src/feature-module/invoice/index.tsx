...
const Invoices = () => {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `http://127.0.0.1:8000/api/invoices/${site_id}/`;
      ...
      let statsUrl = `http://127.0.0.1:8000/api/invoices/${site_id}/?page_size=10000`;
      if (role === "organization" && admin_id) {
        statsUrl += `&admin_id=${admin_id}`;
      }
  ...
  const handleDelete = async (invoiceId: number) => {
    try {
      ...
      // Organization role: admin_id should be passed as query param
      const role = sessionStorage.getItem("role");
      let url = `http://127.0.0.1:8000/api/invoices/${site_id}/${invoiceId}/`;
      if (role === "organization" && admin_id) {
        url += `?admin_id=${admin_id}`;
      }
