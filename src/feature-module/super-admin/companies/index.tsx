...
import { getAdminIdForApi, getSelectedSiteId } from '../../../core/utils/apiHelpers';
import { removeAllBackdrops, resetBodyStyles, cleanupExcessBackdrops, createBackdropCleanupInterval, openModal } from "../../../core/utils/modalHelpers";
...
import AssignSiteModal from "../../hrm/site-assignment/AssignSiteModal";
...
const Companies = () => {
  ...
  const [deactivatedPagination, setDeactivatedPagination] = useState({
    total_items: 0,
    total_pages: 1,
    current_page: 1,
    page_size: 20,
  ...
  const [isEnablingPhotoRefresh, setIsEnablingPhotoRefresh] = useState(false);
  const [isPhotoUpdated, setIsPhotoUpdated] = useState<boolean>(true);
  ...
  // Site assignment modal state
  const [selectedEmployeeForSite, setSelectedEmployeeForSite] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const fetchCompanies = async () => {
      ...
      // For admin role, admin_id comes from request.user, for organization role, pass as query param
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}staff-list/${site_id}/?page=${currentPage}&page_size=${pageSize}`;
      ...
      if (response.data.count !== undefined || response.data.total_objects !== undefined) {
        ...
        // Update current page if backend returned a different page number
  ...
  const fetchDeactivatedEmployees = async (search: string = "") => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}deactivate_list/?page=${deactivatedPagination.current_page}&page_size=${deactivatedPagination.page_size}`;
      if (role === "organization" && admin_id) {
        url += `&admin_id=${admin_id}`;
      }
      if (search) {
      ...
      // Handle response structure similar to staff-list API
      if (response.data && response.data.results && Array.isArray(response.data.results)) {
        deactivatedEmployees = response.data.results;
      ...
      // Set pagination for deactivated employees from backend response
      if (response.data.count !== undefined || response.data.total_objects !== undefined) {
        const totalItems = response.data.count || response.data.total_objects || 0;
        const totalPages = response.data.total_pages || Math.ceil(totalItems / deactivatedPagination.page_size) || 1;
        const currentPageNum = response.data.current_page_number || deactivatedPagination.current_page;
        
        setDeactivatedPagination({
          total_items: totalItems,
          total_pages: totalPages,
          current_page: currentPageNum,
          page_size: response.data.page_size || deactivatedPagination.page_size,
          has_next: response.data.has_next !== undefined ? response.data.has_next : (response.data.next !== null),
          has_previous: response.data.has_previous !== undefined ? response.data.has_previous : (response.data.previous !== null)
        });
        
        // Update current page if backend returned a different page number
        if (currentPageNum !== deactivatedPagination.current_page) {
          setDeactivatedPagination(prev => ({ ...prev, current_page: currentPageNum }));
        }
      }
  ...
  useEffect(() => {
    if (activeTab === 'deactivated') {
      setDeactivatedPagination(prev => ({ ...prev, current_page: 1 }));
  ...
  // Reset deactivated page when search changes and fetch
  useEffect(() => {
    if (activeTab === 'deactivated') {
      setDeactivatedPagination(prev => ({ ...prev, current_page: 1 }));
      // Debounce search
  ...
  // Fetch deactivated employees when pagination changes
  useEffect(() => {
    if (activeTab === 'deactivated') {
      fetchDeactivatedEmployees(searchQuery);
    }
  }, [deactivatedPagination.current_page, deactivatedPagination.page_size]);
  ...
  const handleBulkActivateDeactivate = async (action: 'activate' | 'deactivate') => {
    ...
    try {
      ...
      // Organization role: admin_id should be passed as query param
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}deactivate_list_update/`;
      if (role === "organization" && admin_id) {
        url += `?admin_id=${admin_id}`;
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "username",
      render: (text: string, record: any) => {
        // Handle both formats: nested user object or top-level field
        return record.user?.username || record.username || "-";
      },
      sorter: (a: any, b: any) => {
        const aUsername = a.user?.username || a.username || "";
        const bUsername = b.user?.username || b.username || "";
    ...
    },
    {
      ...
      sorter: (a: any, b: any) => {
        const aEmail = a.user?.email || a.email || "";
        const bEmail = b.user?.email || b.email || "";
        return aEmail.length - bEmail.length;
      },
    },
    {
      title: "Designation",
      ...
      sorter: (a: any, b: any) => {
        ...
        return aDes.length - bDes.length;
      },
    },
    {
      title: "Created At",
      ...
      render: (_: any, record: any) => (
        <button
          ...
          onClick={(e) => {
            ...
            console.log("Selected employee for site assignment:", record);
            
            // Ensure we have the employee object with proper structure
            const employeeRecord = {
              ...
              // Also keep direct ID references
              id: record.user?.id || record.user_id || record.id,
              user_id: record.user?.id || record.user_id || record.id,
            };
            
            console.log("Processed employee record:", employeeRecord);
            
            // Set employee first, then open modal after state update
            setSelectedEmployeeForSite(employeeRecord);
  ...
  const handleEditEmployee = (employee: any) => {
    ...
    const phone_number = employee.phone_number || employee.user?.phone_number || "";
    const is_active = employee.is_active !== undefined ? employee.is_active : (employee.user?.is_active !== undefined ? employee.user.is_active : true);
    ...
    // Set photo update status
    setIsPhotoUpdated(employee.is_photo_updated !== undefined ? employee.is_photo_updated : true);
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      // Add site_id to FormData if available
      const site_id = getSelectedSiteId();
      if (site_id) {
      ...
      // Organization role: admin_id should be passed as query param
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}bulk-register/employees/`;
      if (role === "organization" && admin_id) {
        url += `?admin_id=${admin_id}`;
      }
      ...
      // Handle validation errors (400) - all errors, no records created
      if (response.data?.status === 400) {
        const errors = response.data?.errors || [];
        // Store all errors in state to display in modal
        setBulkUploadErrors(errors);
        // Show toast with summary
        toast.error(
          `${response.data?.message || "Validation failed"}. Please check errors below.`,
          {
            autoClose: 5000,
          }
        );
        // Don't close modal - let user fix errors and try again
        setUploadProgress("");
        setIsUploading(false);
      }
      // Handle success (200) - all validations passed, all records created
      else if (response.data?.status === 200) {
        const processed = response.data?.processed || 0;
        toast.success(`Successfully created ${processed} employee(s)`);
      ...
      if (error.response?.status === 400 && error.response?.data) {
        ...
        if (errors.length > 0) {
          ...
          toast.error(
            ...
            {
              autoClose: 15000, // Show for 15 seconds to read errors
  ...
  const handleUpdateEmployee = async () => {
    ...
    try {
      ...
      // Organization role: admin_id should be passed as query param
      const role = sessionStorage.getItem("role");
      let url = `${BACKEND_PATH}employee_profile_update/${user_id}/`;
      if (role === "organization" && admin_id) {
        url += `?admin_id=${admin_id}`;
      }

      // Use EmployeeProfileUpdateAPIView endpoint with user_id
      const response = await axios.put(
        url,
        {
      ...
      if (error.response?.data) {
        ...
        if (errorData.errors && typeof errorData.errors === 'object') {
          ...
          if (errorData.errors.user) {
            ...
            if (userErrors.email) errorsList.push(`Email: ${Array.isArray(userErrors.email) ? userErrors.email[0] : userErrors.email}`);
            if (userErrors.username) errorsList.push(`Username: ${Array.isArray(userErrors.username) ? userErrors.username[0] : userErrors.username}`);
            if (userErrors.phone_number) errorsList.push(`Phone Number: ${Array.isArray(userErrors.phone_number) ? userErrors.phone_number[0] : userErrors.phone_number}`);
          }
  ...
  const handleTogglePhotoUpdate = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!editingEmployee) return;
    ...
    setIsEnablingPhotoRefresh(true);
    try {
      const token = sessionStorage.getItem("access_token");
      if (!token) {
        toast.error("Please login again");
        setIsEnablingPhotoRefresh(false);
        return;
      }

      // Use PATCH to toggle photo update status
      const response = await axios.patch(
        `${BACKEND_PATH}photo-refresh/${user_id}`,
        { is_photo_updated: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response.data?.status === 200 || response.status === 200) {
        setIsPhotoUpdated(newStatus);
        toast.success(response.data?.message || (newStatus ? "Photo status updated successfully" : "Photo update required enabled successfully"));
      } else {
        toast.error(response.data?.message || "Failed to toggle photo update status");
      }
    } catch (error: any) {
      console.error("Error toggling photo update status:", error);
      const errorMessage = error.response?.data?.message || error.message || "Failed to toggle photo update status";
      toast.error(errorMessage);
    } finally {
      setIsEnablingPhotoRefresh(false);
    }
  ...
  return (
    <>
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const role = sessionStorage.getItem("role");
                      let url = `${BACKEND_PATH}staff-list/${site_id}/?export=true`;
                      if (role === "organization" && admin_id) {
                        url += `&admin_id=${admin_id}`;
                      }
                      ...
                      const link = document.createElement('a');
                      link.href = url_blob;
                      link.setAttribute('download', `employees_export_${admin_id}.xlsx`);
                      document.body.appendChild(link);
                      link.click();
                      link.remove();
              ...
              <div className="tab-content p-3">
                ...
                {activeTab === 'active' ? (
                  ...
                  <>
                    ...
                    pagination={false}
                  />
                  {deactivatedPagination.total_items > 0 && (
                    <div className="d-flex align-items-center justify-content-between mt-3 pt-3 border-top">
                      <div className="d-flex align-items-center">
                        <span className="me-3">Showing {((deactivatedPagination.current_page - 1) * deactivatedPagination.page_size) + 1} to {Math.min(deactivatedPagination.current_page * deactivatedPagination.page_size, deactivatedPagination.total_items)} of {deactivatedPagination.total_items} entries</span>
                        <select 
                          className="form-select form-select-sm" 
                          style={{ width: 'auto' }}
                          value={deactivatedPagination.page_size}
                          onChange={(e) => {
                            setDeactivatedPagination(prev => ({
                              ...prev,
                      ...
                      <div className="d-flex align-items-center">
                        <button
                          className="btn btn-sm btn-white me-2"
                          disabled={!deactivatedPagination.has_previous}
                          onClick={() => {
                            setDeactivatedPagination(prev => ({ ...prev, current_page: prev.current_page - 1 }));
                          }}
                        >
                          <i className="ti ti-chevron-left me-1"></i>
                          Previous
                        ...
                        <span className="me-2">
                          Page {deactivatedPagination.current_page} of {deactivatedPagination.total_pages}
                        </span>
                        <button
                          className="btn btn-sm btn-white"
                          disabled={!deactivatedPagination.has_next}
                          onClick={() => {
                            setDeactivatedPagination(prev => ({ ...prev, current_page: prev.current_page + 1 }));
                          }}
                        >
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    // Password is auto-generated as custom_employee_id@123
                    ...
                    // Build payload (password auto-generated as custom_employee_id@123 in backend)
                    const payload: any = {
                      user: {
                    ...
                    // Add site_id to payload (for admin role - use selected site, for organization - use selected site)
                    const site_id = getSelectedSiteId();
                    if (site_id) {
                    ...
                    if (error.response?.data) {
                      ...
                      if (errorData.errors && typeof errorData.errors === 'object') {
                        ...
                        if (errorData.errors.user) {
                          ...
                          if (userErrors.username) errorsList.push(`Username: ${Array.isArray(userErrors.username) ? userErrors.username[0] : userErrors.username}`);
                          if (userErrors.phone_number) errorsList.push(`Phone Number: ${Array.isArray(userErrors.phone_number) ? userErrors.phone_number[0] : userErrors.phone_number}`);
                          if (userErrors.password) errorsList.push(`Password: ${Array.isArray(userErrors.password) ? userErrors.password[0] : userErrors.password}`);
                        }
                      ...
                      // Handle DRF validation errors (user object directly)
                      if ((!errorMessage || errorMessage === "Failed to register employee") && errorData.user) {
                      ...
                      // Handle top-level field errors (including admin_id, state, city)
                      if (!errorMessage || errorMessage === "Failed to register employee") {
                        ...
                        if (errorData.state) fieldErrors.push(`State: ${Array.isArray(errorData.state) ? errorData.state[0] : errorData.state}`);
                        if (errorData.city) fieldErrors.push(`City: ${Array.isArray(errorData.city) ? errorData.city[0] : errorData.city}`);
                        ...
                        if (errorData.phone_number) fieldErrors.push(`Phone Number: ${Array.isArray(errorData.phone_number) ? errorData.phone_number[0] : errorData.phone_number}`);
                  ...
                  <div className="col-md-6">
                    <div className="mb-3">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setEmployeeForm({ ...employeeForm, state: e.target.value })
                        }
                        required
                      >
                  ...
                  <div className="col-md-6">
                    <div className="mb-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEmployeeForm({ ...employeeForm, city: e.target.value })
                        }
                        required
              ...
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <form onSubmit={(e) => { e.preventDefault(); handleUpdateEmployee(); }}>
                    <div className="row">
                      ...
                      <div className="col-md-6">
                        <div className="mb-3">
                          <label className="form-label">Is Photo Updated</label>
                          <div>
                            <label className="form-check form-switch">
                              <input
                                className="form-check-input"
                                type="checkbox"
                                checked={isPhotoUpdated}
                                onChange={handleTogglePhotoUpdate}
                                disabled={isEnablingPhotoRefresh || isUpdatingEmployee}
                              />
                              <span className="form-check-label">
                                {isPhotoUpdated ? (
                                  <span className="text-success">Updated</span>
                                ) : (
                                  <span className="text-warning">Update Required</span>
                                )}
                              </span>
                              {isEnablingPhotoRefresh && (
                                <span className="ms-2">
                            ...
                            <small className="form-text text-muted d-block mt-1">
                              {!isPhotoUpdated 
                                ? "Employee will need to take selfie on next login" 
                                : "Photo is up to date"}
                            </small>
                          </div>
                        </div>
                ...
                <div className="modal-footer">
                  <button
                    ...
                    className="btn btn-light me-2"
                    onClick={handleCloseEditEmployeeModal}
                    disabled={isUpdatingEmployee || isEnablingPhotoRefresh}
                  >
                    Cancel
                  </button>
                  <button
      ...
      {/* Assign Site Modal */}
      <AssignSiteModal
        employee={selectedEmployeeForSite}
        onSiteAssigned={(result) => {
          toast.success("Site assigned successfully!");
          setSelectedEmployeeForSite(null);
          fetchCompanies(); // Refresh employee list
        }}
        onClose={() => setSelectedEmployeeForSite(null)}
      />
      {/* /Assign Site Modal */}
